...
public class FeatureList extends Lister<Feature> {
	...
	protected List<Feature> doMap(HttpServletRequest request, ResultSet resultSet) throws Exception {
		...
		while (resultSet.next()) {
			...
			Instrument instrument = new Instrument(instrumentId, instrumentUri, instrumentName, null); 
			Measure measure = new Measure(measureId, measureName, measureUnit, null);
			Feature feature = new Feature(id, name, topic, factor, measure, instrument, null);
