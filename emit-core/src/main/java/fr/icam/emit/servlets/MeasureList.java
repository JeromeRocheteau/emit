package fr.icam.emit.servlets;

import java.sql.ResultSet;
import java.util.LinkedList;
import java.util.List;

import javax.servlet.http.HttpServletRequest;

import fr.icam.emit.entities.Measure;

public class MeasureList extends Lister<Measure> {

	private static final long serialVersionUID = 201708241432001L;

	@Override
	protected List<Measure> doMap(HttpServletRequest request, ResultSet resultSet) throws Exception {
		List<Measure> measures = new LinkedList<Measure>();
		while (resultSet.next()) {
			Long id = resultSet.getLong("id");
			String name = resultSet.getString("name");
			String unit = resultSet.getString("unit");
			Measure measure = new Measure(id, name, unit, null);
			measures.add(measure);
		}
		return measures;
	}
	
}